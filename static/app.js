...
class SuperBizAgentApp {
    ...
    bindEvents() {
        ...
        if (this.chatContainer) {
            this.chatContainer.addEventListener('dragover', (e) => {
                e.preventDefault();
                this.chatContainer.classList.add('drag-over');
            });
            this.chatContainer.addEventListener('dragleave', () => {
                this.chatContainer.classList.remove('drag-over');
            });
            this.chatContainer.addEventListener('drop', (e) => {
                e.preventDefault();
                this.chatContainer.classList.remove('drag-over');
                if (e.dataTransfer.files.length > 0 && this.fileInput) {
    ...
    async triggerTaskTemplate(templateKey) {
        const labels = {cpu_alert:'CPU 告警分析', memory_alert:'内存泄漏排查', slow_response:'慢响应排查', service_down:'服务不可用'};
        ...
        try {
            ...
            this.addMessage('assistant', '任务执行失败: ' + e.message);
        }
        this.hideLoading();
    }
